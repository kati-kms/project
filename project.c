...
//  int 우선순위 ()
//  int 변수()
	int cipher_save_stack_integers(int c, int stack[N],int count); //stack에 저장해둔다. ( ' ' 전까지 space바 전까지)
    int cipher_save_num_integers(int num[][N], int stack[N], int count); // count를 매개변수로 자릿수 함수만든다. num변수에 넣어줄때마다 호출?
	int cipher_save_num_decimal_places(int c, int num[][N], int i); // num i번쨰 [N]을 받아서 (숫자한줄)  소수부분을 넣는다.
	...
	while((c=getchar())!='\n')//엔터키 전까지 다 넣기
	{
		if(c!=' ')//숫자,연산자  경계   마다 끊기 (숫자 / 연산자 )
		{
			if(c=='1'||c=='2'||c=='3'||c=='4'||c=='5'||c=='6'||c=='7'||c=='8'||c=='9'||c=='0') // 숫자일경우
			{
				...
				{
					cipher_save_num_decimal_places(c,num[i],i);   // 소수부분
				}
				else
					count=cipher_save_stack_integers(c,stack,count);// 우선은stack에넣고 count를 리턴하여 stop_count에 넣음 // 검토사항 count변수 나눌까??
		...
		{
		cipher_save_num_integers(num[i],stack,count); //stack부분  정수부분  넣기 //숫자의 경우 소수점이 없는애들이 끝나는시점이니까 stack을 num에 넣어야하나요?
		space_flag++;
...
int cipher_save_stack_integers(int c, int stack[],int count) { //stack에 저장해둔다. ( ' ' 전까지 space바 전까지)
...
int cipher_save_num_integers(int num[][N], int stack[], int count) { //stack한테 자리수도 받았고,stack을 그냥 num에 넣기만하면됨   현)stack에깔끔하게 다들어가있음
